...
.then(data => {
  ...
  if (data.role === "fizjo") {
    ...
    if (!hasTrainings && !hasAppointments) {
      container.textContent = "Nie masz żadnych zapisów na treningi ani wizyt.";
      return;
    }
    ...
    if (hasTrainings) {
      ...
      data.data.trainings.forEach(training => {
        ...
        cancelButton.onclick = () => cancelItem(training.TrainingID, 'training');
    ...
    if (hasAppointments) {
      ...
      data.data.appointments.forEach(appointment => {
        ...
        cancelButton.className = "cancel-btn";
        cancelButton.onclick = () => cancelItem(appointment.AppointmentID, 'appointment');
