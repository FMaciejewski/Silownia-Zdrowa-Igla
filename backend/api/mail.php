...
function sendEmail($to, $subject, $body, $fromEmail, $appPassword) {
    ...
    if (substr($response, 0, 3) != '220') return "Invalid greeting: $response";

    fputs($socket, "EHLO $host\r\n");
    $response = fgets($socket, 512);
    while (substr($response, 3, 1) == '-') $response = fgets($socket, 512);
    if (substr($response, 0, 3) != '250') return "EHLO failed: $response";
    ...
    $response = fgets($socket, 512);
    while (substr($response, 3, 1) == '-') $response = fgets($socket, 512);
    if (substr($response, 0, 3) != '250') return "EHLO after TLS failed: $response";

    fputs($socket, "AUTH LOGIN\r\n");
    $response = fgets($socket, 512);
    if (substr($response, 0, 3) != '334') return "AUTH LOGIN failed: $response";
    ...
    $response = fgets($socket, 512);
    if (substr($response, 0, 3) != '235') return "Password rejected: $response";
